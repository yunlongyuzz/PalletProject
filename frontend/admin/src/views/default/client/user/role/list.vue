  ...
  <d2-container>
    <div class="header-container">
      <el-row :gutter="20">
        <el-col :md="6">
          <div>
            <default-client-user-role-add class="filter-item" @addDone="getListItems"/>
            </div>
        </el-col>
      </el-row>
    </div>
    ...
    <default-client-user-role-edit :id="infoEditId" @editDone="getListItems" @editClosed="infoEditId=null"/>
    <default-client-user-role-view :id="infoViewId" @viewClosed="infoViewId=null"/>
    <!--
...
import DefaultClientUserRoleAdd from './add'
import DefaultClientUserRoleEdit from './edit'
import DefaultClientUserRoleView from './view'
export default {
  name: 'Default_Client_User_Role_List',
  components: {
    DefaultClientUserRoleAdd,
    DefaultClientUserRoleEdit,
    DefaultClientUserRoleView
  },
  filters: {
  ...
  methods: {
    getListItems () {
      ...
      this.$api.table.getList('Default_Client_User_Role', this.listQuery).then(response => {
    ...
    handleDelete (id) {
      ...
      MessageBox.confirm(
        ...
        that.$api.table.delInfo('Default_Client_User_Role', { id }).then((response) => {
          that.getListItems()
        })
      })
    }
