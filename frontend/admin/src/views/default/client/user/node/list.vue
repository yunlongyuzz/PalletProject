  ...
  <d2-container>
    <div class="header-container">
      <el-row :gutter="20">
        <el-col :md="6">
          <div>
            <default-client-user-node-add class="filter-item" @addDone="getListItems"/>
            </div>
        </el-col>
      </el-row>
    </div>
    ...
    <default-client-user-node-edit :id="infoEditId" @editDone="getListItems" @editClosed="infoEditId=null"/>
    <default-client-user-node-view :id="infoViewId" @viewClosed="infoViewId=null"/>
    <!--
...
import DefaultClientUserNodeAdd from './add'
import DefaultClientUserNodeEdit from './edit'
import DefaultClientUserNodeView from './view'
export default {
  name: 'Default_Client_User_Node_List',
  components: {
    DefaultClientUserNodeAdd,
    DefaultClientUserNodeEdit,
    DefaultClientUserNodeView
  },
  filters: {
  ...
  methods: {
    getListItems () {
      ...
      this.$api.table.getList('Default_Client_User_Node', this.listQuery).then(response => {
    ...
    handleDelete (id) {
      ...
      MessageBox.confirm(
        ...
        that.$api.table.delInfo('Default_Client_User_Node', { id }).then((response) => {
          that.getListItems()
        })
      })
    }
